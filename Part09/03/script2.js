...
$(document).ready(function() {
	...
	$.prepDB = function() {
		if (window.openDatabase) {
			try {
				db = openDatabase('addressbook',										// DB명
				 				  '1.0',												// DB버전
								  '주소록 저장공간',									// DB설명
	...
	$.searchFormSubmitted = function(event) {
		...
		// 3. 애니메이션 시작 부분 넣기
		$("#spinner").bind('webkitAnimationEnd',function(event) {
			$.spinnerAnimationEnded(event)
		});
		$("#spinner").attr('class','spin');
	
		return false;
	}
	
	$.spinnerAnimationEnded = function(event) {
	...
	$.addContact = function(event) {
		...
		$.ajax({
			...
			success: function(data){
				...
				twitterImagePath = profile[0].user.profile_image_url;
